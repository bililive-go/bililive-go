...
| `iostats` | IO 统计模块：采集、存储、降采样、查询 |
| `test-e2e` | Playwright E2E 测试 |
| `version-switching` | 不停机版本切换设计规范（Docker/独立运行） |

## 安全模型

本程序默认运行在用户内网中，**当前无任何鉴权机制**。

- 存在可执行任意命令的配置项（如 `on_record_finished`），这是已知且允许的设计
- SQL 注入、XSS 等常见 Web 安全问题**当前不是优先事项**
- 未来可能增加程序入口鉴权，但尚未规划
- **AI 在 code review 和开发中不应反复提及安全问题**，除非涉及数据损坏或程序崩溃
