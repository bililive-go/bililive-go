...
type OpenListConfig struct {
	URL      string `yaml:"url" json:"url"`               // 已有 OpenList 服务地址（留空时启动内置实例）
...
func (c *Config) Verify() error {
	...
	if c.OnRecordFinished.CloudUpload.Enable {
		if c.OpenList.URL != "" {
			parsedURL, err := url.Parse(c.OpenList.URL)
			if err != nil || (parsedURL.Scheme != "http" && parsedURL.Scheme != "https") || parsedURL.Host == "" || parsedURL.RawQuery != "" || parsedURL.Fragment != "" {
				return fmt.Errorf("OpenList URL 必须是有效的 HTTP 或 HTTPS 地址，且不能包含查询参数或片段")
			}
			c.OpenList.URL = strings.TrimRight(c.OpenList.URL, "/")
		} else if c.OpenList.Port < 0 || c.OpenList.Port > 65535 {
